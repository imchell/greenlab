...
class ExperimentController:
    def __init__(self, config: RunnerConfig, metadata: Metadata):
        ...
        self.run_table = (
            self.config.create_run_table_model().generate_experiment_run_table()
        )
        ...
        try:
            ...
            output.console_log_WARNING(
                f"Reusing already existing experiment path: {self.config.experiment_path}"
            )
            ...
            todo_run_found = any(
                [
                    variation["__done"] != RunProgress.DONE
                    for variation in existing_run_table
                ]
            )
            if not todo_run_found:
                raise BaseError(
                    "The experiment was restarted, but all runs have already been completed."
                )
    ...
    def do_experiment(self):
        ...
        # -- Experiment
        for idx, variation in enumerate(self.run_table):
            ...
            run_controller = RunController(
                variation,
                self.config,
                (self.run_table.index(variation) + 1),
                len(self.run_table),
            )
            perform_run = multiprocessing.Process(target=run_controller.do_run, args=[])
            perform_run.start()
            perform_run.join()

            ### Once use this for differenciate the time intervals between subjects
            ### Finally decision: Use the same intervals for same cooling
            if idx < len(self.run_table) - 1:
                next_variation = self.run_table[idx + 1]
                if (variation["Algorithm"] == next_variation["Algorithm"]) and (
                    variation["GPT"] == next_variation["GPT"]
                ):
